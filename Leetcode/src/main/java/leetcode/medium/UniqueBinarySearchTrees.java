...
public class UniqueBinarySearchTrees {
    ...
    /**
     * Reference link https://discuss.leetcode.com/topic/8398/dp-solution-in-6-lines-with-explanation-f-i-n-g-i-1-g-n-i
    ...
    public int numTrees(int n) {
        ...
        G[0] = G[1] = 1;
        /**
         * Key part is come up with the G(n) = F(1,n)+f(2,n) + f(n,n)
         * F(i,n) = G(i-1)*G(n-i);
         * Then G(n) = G.......G...
         * So we get the dp function.
         * */
        for(int i = 2; i <= n; i++){
            for(int j = 1; j <= i; j++){
                G[i] += G[j-1]*G[i-j];
